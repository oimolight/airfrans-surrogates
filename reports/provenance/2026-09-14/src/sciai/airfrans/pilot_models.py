...
def make_pilot_model(model_name: str) -> nn.Module:
    if model_name == "mlp":
        return PointwiseMLP(input_features=6, hidden_width=64, hidden_layers=2)
    if model_name == "gnn":
        return PilotGNN(input_features=6, hidden_width=64, blocks=4)
    if model_name == "fno":
        return PilotFNO(input_features=6, width=32, blocks=4, modes=12)
