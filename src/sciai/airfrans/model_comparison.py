...
from .metrics import select_validation_checkpoint
from .models import (
...
EXPERIMENT_ID = "airfrans-model-comparison"
MODEL_NAMES = ("mlp", "gnn", "fno")
FIT_STATUS = "VALIDATION_SELECTED"
LOCK_SCHEMA_VERSION = 1
PROVENANCE_ROOT = Path("reports/provenance/2026-09-14")
EVALUATOR_SOURCE_FILES = (
    Path("src/sciai/airfrans/model_comparison.py"),
    Path("src/sciai/airfrans/data.py"),
    Path("src/sciai/airfrans/metrics.py"),
    Path("src/sciai/airfrans/models.py"),
    Path("src/sciai/airfrans/sampling.py"),
)


class ModelComparisonTimeout(RuntimeError):
    pass
...
def timeout_handler(signum: int, frame: object) -> None:
    raise ModelComparisonTimeout("Model-comparison run exceeded its frozen wall-time limit")
...
def validate_protocol(base_config: Mapping[str, Any], selection_path: Path) -> dict[str, list[str]]:
    ...
    if base_config["seeds"] != [17, 29, 41]:
        raise RuntimeError("Model-comparison seeds changed")
    training = base_config["training"]
    if training["epochs"] != 100 or training["validation_interval_epochs"] != 10:
        raise RuntimeError("Model-comparison schedule changed")
...
def make_model(model_name: str, config: Mapping[str, Any]) -> nn.Module:
    ...
    raise ValueError(f"Unsupported model-comparison model: {model_name}")
...
def locked_source_hashes_match(
    ...
    for relative_path, expected_hash in locked_hashes.items():
        candidates = (
            ...
            repository_root / PROVENANCE_ROOT / relative_path,
        )
        if not any(path.is_file() and file_sha256(path) == expected_hash for path in candidates):
            return False
    return True
...
def fit_model(
    ...
    if model_name not in MODEL_NAMES or seed not in base_config["seeds"]:
        raise ValueError("Model or seed is not part of the frozen comparison matrix")
...
def evaluate_locked_test_run(
    ...
    if not locked_source_hashes_match(repository_root, lock.get("evaluation_source_sha256", {})):
...
def evaluate_all_locked_tests(
    ...
    for entry in lock["entries"]:
        command = [
            sys.executable,
            "-m",
            "sciai.airfrans.model_comparison",
            "test-one",
            "--run-id",
            entry["run_id"],
            "--lock",
...
def create_summary(
    ...
    lines = [
        ...
        "Status: COMPLETED",
        "",
        "Checkpoints were selected solely by case-macro nondimensional pressure RMSE on the 40 validation cases.",
        "After all nine checkpoints were locked, each checkpoint was evaluated exactly once on the frozen 50-case distribution-ID test set.",
        "The reference quantity for nondimensional errors was `0.5 * |U_inlet|^2` for each case and was not derived from test labels.",
        "GNN neighborhoods and airfoil-crossing checks used raw physical coordinates, while relative displacements in messages used coordinates standardized with training statistics.",
        "",
    ...
    lines.extend(
        [
            ...
            "## Limitation",
            "",
            "This comparison uses the frozen distribution-ID / unseen-exact-geometry split and does not measure geometry-OOD performance.",
            "Even with the same evaluation points and budget, the grid, graph, receptive field, and parameter count differ, so the results do not isolate the causal effect of architecture alone.",
            "Geometry-OOD evaluation was not performed.",
            "",
        ]
    )
...
def run_all_subprocesses(args: argparse.Namespace) -> None:
    ...
    if failures:
        raise RuntimeError(f"Model-comparison fits failed; test remains locked: {failures}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/airfrans/model_comparison.json"))
    ...
    parser.add_argument("--summary-json", type=Path, default=Path("reports/summaries/model_comparison_summary.json"))
    parser.add_argument("--summary-markdown", type=Path, default=Path("reports/summaries/model_comparison_results.md"))
